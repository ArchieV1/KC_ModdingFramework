...
namespace Zat.InterModComm
{
    public class IMCPort : MonoBehaviour
    {
        ...
        private void ReceiveMessage(object[] data)
        {
            ...
            else
            {
                ...
                {
                    throw new Exception($"Unhandled request: {message}");
                }
                else
                {
        ...
        public void RPC<P>(string target, string name, P payload, float timeout, UnityAction callback, UnityAction<Exception> error)
        {
            try
            {
                var message = IMCMessage.CreateRequest(this.name, name, JsonConvert.SerializeObject(payload, serializerSettings));
    ...
    class IMCMessage
    {
        ...
        {
            return new IMCMessage(source, Name, MessageType.Response, ID, payload != null ? JsonConvert.SerializeObject(payload, IMCPort.serializerSettings) : null);
        }
        /// <summary>
        ...
        public T ParsePayload<T>()
        {
            ...
            try
            {
                return JsonConvert.DeserializeObject<T>(Payload, IMCPort.serializerSettings);
            }
            catch
            {
                return default(T);
